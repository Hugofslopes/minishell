...
/*   Updated: 2025/04/07 13:05:20 by hfilipe-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../includes/minishell.h"

int	check_export_errors2(char *input, t_ms **ms, size_t check_eq, size_t i)
{
	while (input[i])
	{
		if (input[i] == '=')
...
int	check_export_errors(char *input, t_ms **ms, size_t i, size_t check_eq)
{
	while (input[i])
	{
		if (i == 0 && input[i] == '+')
		...
		if (ft_isdigit(input[i]) && i == 0)
			...
			return (ft_put_error(ERROR_EXP_P, "%s", input), (*ms)->exit_status \
			= 1, 1);
		else if (!check_eq && ft_strchr(input, '\'') && ft_strchr(input, '$'))
		{
			if (check_dollar_before_eq(input, 0, 0, 0))
				return ((*ms)->exit_status = 1, 1);
		}
		else
			if (check_export_errors2(input, ms, check_eq, 0))
				return (1);
		i++;
	}
	return (0);
...
int	check_e(t_ms **ms)
{
	...
	while ((*ms)->export[i])
	{
		if (check_export_errors((*ms)->export[i], ms, 0, 0))
		{
			realoc_export(ms, i);
			return (1);
		}
